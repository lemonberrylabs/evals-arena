      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${providerBgColor}`}>{modelResponse.provider}</span>
            {isWinner && (
        ...
        {judgeEvaluation && (
          <div className="mt-2 text-sm text-gray-700 bg-gray-50 p-2 rounded">
            <p>
              <strong>Judge:</strong> {judgeEvaluation.reasoning}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="p-4 bg-gray-50 border-t border-gray-100">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex gap-2">
            <button onClick={handleCopy} className="text-gray-500 hover:text-gray-700" title="Copy response">
        ...
        <div className={`prose prose-sm max-w-none ${!expanded ? 'max-h-64 overflow-y-hidden' : ''}`}>
          <div className="whitespace-pre-wrap">{modelResponse.response}</div>
        </div>
        ...
        {!expanded && (
          <div className="relative -mt-8 pt-8 pb-2 text-center">
            ...
            <button onClick={() => setExpanded(true)} className="text-sm text-gray-500 hover:text-gray-800">
