...
export function BattleForm({ onSubmit, isLoading }: BattleFormProps) {
    ...
    const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<BattleFormValues>({
                    ...
                    {activeTab === 'user' && (
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    className={`w-full h-64 px-3 py-2 border rounded-md ${errors.userPrompt ? 'border-red-500' : 'border-gray-300'}`}
                                    placeholder="Enter the prompt you want to evaluate the models on..."
                                    {...register('userPrompt')}
                                />
                                {errors.userPrompt && (
                                    <p className="mt-1 text-sm text-red-500">{errors.userPrompt.message}</p>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'judge' && (
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    className={`w-full h-64 px-3 py-2 border rounded-md ${errors.judgeCriteria ? 'border-red-500' : 'border-gray-300'}`}
                                    placeholder="Enter criteria for judging the model responses..."
                                    {...register('judgeCriteria')}
                                />
                                {errors.judgeCriteria && (
                                    <p className="mt-1 text-sm text-red-500">{errors.judgeCriteria.message}</p>
                                )}
                            </div>
                        </div>
                    )}
            ...
            <div className="flex items-center justify-between">
                <div>
                    {errors.selectedModels && (
                        <p className="text-sm text-red-500">{errors.selectedModels.message}</p>
                    )}
                </div>
                <Button
                    type="submit"
